...
public class BonfireBehaviour : MonoBehaviour
{
    ...
    void checkPlayerRest() 
    {
        ...
        {
            // add prompt to rest
            bonfireRestText.SetText("NHẤN H ĐỂ AN TỌA NGHỈ NGƠI BÊN ĐỐM LỬA");
            pState.canRest = true;
        }
        
        if (pState.resting) 
        {
            bonfireRestText.SetText("NHẤN H ĐỂ BẬT DẬY VÀ TIẾP TỤC CHIẾN ĐẤU!");
        }

        if (Input.GetKeyDown(KeyCode.H) && (pState.canRest || pState.resting) && !PauseMenu.GameIsPaused)
        {
            if (!pState.resting) 
            {
