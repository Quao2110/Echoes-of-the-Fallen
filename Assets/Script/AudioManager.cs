...
public class AudioManager : MonoBehaviour
{
    ...
    public AudioClip hKhuHamMoMusic;
    ...
    public void startBossOutro()
    {
        ...
        StartCoroutine(LoadWinSceneAfterMusic(bossFightOutro.length));
    }
    // HÀM MỚI: Tải scene sau khi nhạc kết thúc
    private IEnumerator LoadWinSceneAfterMusic(float musicLength)
    {
        // Chờ cho nhạc outro phát gần hết
        yield return new WaitForSeconds(musicLength - 0.5f); // Trừ 0.5s để mượt hơn

        // (Tùy chọn: Thêm hiệu ứng fade-out ở đây nếu muốn)
